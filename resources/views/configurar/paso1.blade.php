...
@endphp
  <div class="p-6 m-1 grid sm:grid-cols-2 md:grid-cols-1 items-center justify-center" style="background-color:#ffee00;">
    ...
    <div class="ml-1 mt-2">
        @if($user->modo=="novel")
          <a href="{{route('materias.create')}}" title="Añadir Materia" class="bt_pasos oscuro">{{ __('Add') }} {{ __('Subject') }}</a>
        @elseif($user->modo=="avanzado")
          <a href="{{route('materias.createall')}}"  title="Añadir todas las materias" class="bt_pasos oscuro">{{ __('Add all') }} </a>
        @endif
    </div>
  </div>
  ...
  <div class="p-6 m-1 grid sm:grid-cols-2 md:grid-cols-1 items-center justify-center" style="background-color:  hsl(182, 64%, 91%)" >
      ...
      <div class="ml-1 text-lg font-semibold">
       ...
       <h3 class="pasos-title-2"><span class="ico-shadow">📅  </span> {{ __('Timetable')}}</span></h3>
      </div>
