  ...
  <details>
    ...
    <form  id="gradual_btform" class="mx-2" method="POST" action="{{ route('botones.store') }}">
      @csrf
        ...
        <input id="gradual_bt_name" class="d_block" type="text" name="bt_name" value="{{ $plantilla->bt_name }}" />
        @error('bt_name')
            <small class="t_red">* {{ $message }}</small><br>
        @enderror

        <label class= "d_block mt-2" for="descripcion"><strong>Descripción</strong></label>
        <input id="gradual_descripcion"class="d_block" type="text" name="descripcion" value="{{ $plantilla->descripcion }}" />
        @error('descripcion')
            <small class="t_red">* {{ $message }}</small><br>
        @enderror
        <div class="grid grid-cols-2 justify-between">
          <div class="mr-1">
            ...
            <input id="gradual_default" class="d_block" type="text" name="default" value= {{$plantilla->default}} />
            @error('default')
                <small class="t_red">* {{ $message }}</small><br>
            @enderror
          </div>
          <div>
            ...
            <input id="gradual_v_last" class="d_block" type="text" name="v_last" value= {{$plantilla->v_last}} />
            @error('v_last')
                <small class="t_red">* {{ $message }}</small><br>
            @enderror
          </div>
        ...
        <input id="gradual_pasos" class="d_block" type="text" name="pasos" value= {{$plantilla->pasos}} />
        @error('pasos')
            <small class="t_red">* {{ $message }}</small><br>
        @enderror
      <div>
